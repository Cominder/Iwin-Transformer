# --------------------------------------------------------
# Iwin Transformer, built on Swin Transformer
# Licensed under The MIT License for use by researchers and developers.
...
class IwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # rearrange x
        x = rearrange(x, H // self.window_size, W // self.window_size)     # B H' W' C
...
class DepthwiseConv2d(nn.Module):
    ...
    def flops(self):
        flops = 0
        flops += self.dim * self.kernel_size * self.kernel_size
        return flops
...
class Downsample2x(nn.Module):
    ...
    def flops(self):
        ...
        flops += H * W * self.dim
        # conv2d
        flops += 2 * self.dim * self.dim * 9 * (H // 2) * (W // 2)
...
class IwinTransformer(nn.Module):
    r""" Iwin Transformer 
         A PyTorch impl of : `Iwin Transformer: Hierarchical Vision Transformer using Interleaved Windows`  -
    ...
    Args:
        ...
        depths (tuple(int)): Depth of each Iwin Transformer layer.
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 embed_dim=32, depths=(2, 2, 6, 2), num_heads=(4, 8, 16, 32),
                 window_size=7, mlp_ratio=4., qkv_bias=True,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=True, patch_norm=True, pretrained_cfg=None, pretrained_cfg_overlay=None,
