"""
Training Codes of FlashDiT together with VA-VAE.
built on codes from (https://github.com/hustvl/LightningDiT)
...
from models.flashdit import FlashDiT_models
...
def do_train(train_config, accelerator):
    ...
    model = FlashDiT_models[train_config['model']['model_type']](
    ...
    if accelerator.is_main_process:
        logger.info(f"FlashDiT Parameters: {sum(p.numel() for p in model.parameters()) / 1e6:.2f}M")
    ...
    if accelerator.is_main_process:
        ...
        logger.info(f"Effective batch size: {effective_batch_size} (global_batch_size * gradient_accumulation_steps)")
    ...
    while True:
        for x, y in loader:
            ...
            # Log loss values (accumulate for logging regardless of accumulation steps):
            if 'cos_loss' in loss_dict:
                running_loss += (mse_loss.item() / gradient_accumulation_steps)
            else:
                running_loss += (loss.item() * gradient_accumulation_steps)  # Multiply back for accurate logging
            log_steps += 1

            # Only update parameters after accumulating gradients for the specified number of steps
            if accumulation_counter == gradient_accumulation_steps:
                ...
                accumulation_counter = 0
                train_steps += 1  # Only increment train_steps when we actually update parameters

            if train_steps % train_config['train']['log_every'] == 0 and train_steps > 0:
...
if __name__ == "__main__":
    ...
    # Add gradient accumulation steps to config if not present
    if 'gradient_accumulation_steps' not in train_config['train']:
        train_config['train']['gradient_accumulation_steps'] = 8
