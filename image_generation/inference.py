...
def do_sample(train_config, accelerator, ckpt_path=None, cfg_scale=None, model=None, vae=None, demo_sample_mode=False):
    ...
    if demo_sample_mode:
        if accelerator.process_index == 0:
            images = []
            for label in tqdm([975, 21, 203, 387, 356, 99, 979, 277, 388, 3, 949, 355], desc="Generating Demo Samples"):
            ...
            grid = np.zeros((3 * h, 4 * w, 3), dtype=np.uint8)
...
def print_with_prefix(*messages):
    prefix = f"\033[34m[FlashDiT-Sampling {strftime('%Y-%m-%d %H:%M:%S')}]\033[0m"
...
def print_conv_weights(model, ckpt_path):
    """
    Print all Conv2D weights and biases in the model.
    """
    # Load checkpoint
    checkpoint = torch.load(ckpt_path, map_location="cpu")
    if "ema" in checkpoint:
        print_with_prefix("Using EMA weights")
        state_dict = checkpoint["ema"]
    elif "model" in checkpoint:
        print_with_prefix("Using model weights")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', type=str, default='configs/flashdit_b_ldmvae_f16d16.yaml')
